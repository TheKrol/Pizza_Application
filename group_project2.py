...
class StartPage(tk.Frame,PizzaApp):
    ...
    def login_page(self):
        ...
        emp_label = Label(self.login_win,bg="blue", text="Enter your 4 digit employee pin")
        emp_label.pack()
...
class MenuPage(StartPage):
    ...
    def menu_start(self):
        ...
        global breadstick, subtotal_price, tax, delivery_fee, total_price
        global carryout, pick_up, delivery,order_box
        ...
        global toppings, size, sauce, pizza, side, order_type, order_box, price
        ...
        price = [delivery_fee,tax,total_price,subtotal_price]
        ...
        order_box.config(width=45,height=30,borderwidth=0,
            yscrollcommand=scroll_bar.set)
        order_box.place(x=0,y=29)

        #Price lables
        self.total_label=tk.Label(self,text=f"Toatal Price: {total_price:.2f}",bg='blue',font=20,
            borderwidth=0)
        self.total_label.place(x=125,y=530)
        self.sub_label=tk.Label(self,text=f"Subtotal Price: {subtotal_price:.2f}",bg='blue',
            borderwidth=0)
        self.sub_label.place(x=0,y=520)
        self.tax_label=tk.Label(self,text=f"Tax: {tax:.2f}",bg='blue',borderwidth=0)
        self.tax_label.place(x=0,y=535)
        self.del_label=tk.Label(self,text=f"Delivery Fee: {delivery_fee:.2f}",bg='blue',
            borderwidth=0)
        self.del_label.place(x=0,y=550)

        #Main Buttons
        tk.Checkbutton(self,variable=carryout, text="Carryout",borderwidth=0,background='blue',
             font=14,command=self.carryout_fn).place(x=0,y=0)
        tk.Checkbutton(self,variable=pick_up, text="Pick Up",borderwidth=0,background='blue',
            font=14,command=self.pickup_fn).place(x=88,y=0)
        tk.Checkbutton(self,variable=delivery, text="Delivery",borderwidth=0,background='blue',
            font=14,command=self.delivery_fn).place(x=173,y=0)
        ...
        tk.Button(self, image=self.customer,compound="top",text="Customer",bg='blue',borderwidth=0,
            command=lambda: self.controller.show_frame("CustomerPage")).place(x=0,y=570)
        tk.Button(self, image=self.add_item,compound="top",bg='blue',borderwidth=0,
            text="Add Item", command=self.add_item_fn).place(x=200,y=570)
        tk.Button(self, image=self.delete_item,compound="top",text="Delete Item",borderwidth=0,
            bg='blue',command=self.delete_item_fn).place(x=400,y=570)
        tk.Button(self, image=self.submit_order,compound="top",text="Submit Order",borderwidth=0,
            bg='blue',command=self.submit_order_fn).place(x=600,y=570)
        tk.Button(self, image=self.exit,compound="top",bg='blue',borderwidth=0,
            text="Exit", command=self.exit_fn).place(x=800,y=570)
    ...
    def delivery_fn(self):
        ...
        if delivery.get():
            ...
            delivery_fee = 0
        self.del_label.config(text=f"Deliver Fee: {delivery_fee:.2f}")
        total_price = delivery_fee + subtotal_price + tax
        self.total_label.config(text=f"Total Price: {total_price:.2f}")

    #Add items to order function
    def add_item_fn(self):
        """Add item to order"""
        global order_box, subtotal_price, tax, delivery_fee,total_price
        print(f"Pizza frame: {PIZZA_FRAME_VAR}")
        if PIZZA_FRAME_VAR:
                ...
                if i.get():
                    ...
                    if small.get():
                        ...
                        if topping_count <1:
                            ...
                            topping_count-=1
                        subtotal_price +=6.99 +(1.29*topping_count)
                        self.sub_label.config(text=f"Subtotal Price: {subtotal_price:.2f}")
                    if medium.get():
                        for j in toppings:
                            if j.get():
                                topping_count +=1
                        if topping_count <1:
                            ...
                            topping_count-=1
                        subtotal_price +=7.99+(1.49*topping_count)
                        self.sub_label.config(text=f"Subtotal Price: {subtotal_price:.2f}")
                    if large.get():
                        for j in toppings:
                            if j.get():
                        ...
                        if topping_count <1:
                            ...
                            topping_count-=1
                        subtotal_price +=10.99+(1.79*topping_count)
                        self.sub_label.config(text=f"Subtotal Price: {subtotal_price:.2f}")
                    i.set(False)
            for i in pizza:
                if i.get():
        ...
        #Updating Total price and tax
        if delivery.get():
            delivery_fee = 4.95
        tax = subtotal_price *.06
        total_price = tax + subtotal_price + delivery_fee
        self.tax_label.config(text=f"Tax: {tax:.2f}")
        self.total_label.config(text=f"Total Price: {total_price:.2f}")

    def delete_item_fn(self):
        """Function to delete an item"""
        global subtotal_price,tax,delivery,total_price
        topping_count = 0
        del_size = 'None'
        topping_list = ['Pepperoni','Mushroom','Bacon','Sausage','Ham','Green Pepper',
            'Banana Pepper','Onion']
        ...
        for j in order_box.get(item[0], END):
            #Getting topping count
            if isinstance(j, tuple) and delete_itm != item[0]:
                break
            if str(j).strip() in topping_list:
                topping_count +=1
            if isinstance(j, tuple) and str(j[0]) == 'Small':
                #Updating Price for small deletion
                del_size = 'Small'
                subtotal_price -= 6.99
            if isinstance(j, tuple) and str(j[0]) == 'Medium':
                #Updating Price for Medium deletion
                del_size = 'Medium'
                subtotal_price -= 7.99
            if isinstance(j, tuple) and str(j[0]) == 'Large':
                #Updating Price for Large deletion
                del_size = 'Large'
                subtotal_price -= 10.99
        ...
        #Update topping count to be accurate
        if topping_count <=1:
            topping_count = 0
        else:
            topping_count -=1
        print(f'Del size: {del_size}')
        if del_size == 'Small':
            subtotal_price -= (1.29*topping_count)
        elif del_size == 'Medium':
            subtotal_price -= (1.49*topping_count)
        else:
            subtotal_price -= (1.79*topping_count)

        #updating price upon deletion
        tax = subtotal_price *.06
        total_price = subtotal_price + tax + delivery_fee
        self.sub_label.config(text=f"SubTotal Price: {subtotal_price:.2f}")
        self.tax_label.config(text=f"Tax: {tax:.2f}")
        self.total_label.config(text=f"Total Price: {total_price:.2f}")
    ...
    def submit_order_fn(self):
        ...
        global ORDER_NUMBER, CUSTOMER, order_box, employee_name, price
        ...
        if self.write_order_file():
            ...
            tax = subtotal_price *.06
            total_price = subtotal_price + tax + delivery_fee
            self.del_label.config(text=f"Delivery Fee: {delivery_fee:.2f}")
            self.sub_label.config(text=f"SubTotal Price: {subtotal_price:.2f}")
            self.tax_label.config(text=f"Tax: {tax:.2f}")
            self.total_label.config(text=f"Total Price: {total_price:.2f}")
            ORDER_NUMBER +=1
    ...
    def write_order_file(self):
        ...
        global order_box, employee_name, total_price
        #open file
        try:
            file = open(os.path.join(__location__, "logs/Order_File.txt"),
            "a", encoding="utf-8")
        ...
        for i in order_box.get(0, END):
            ...
            file.write(f"{i} ")
        file.write(f"Order total is: {total_price:.2f} ")
    ...
    def exit_fn(self):
        ...
        global employee_name, price,order_box,subtotal_price,delivery_fee,tax,total_price
        #Add logging
        with open(os.path.join(__location__,"logs/employeeslog.txt"),
            mode="a+", encoding="utf-8") as emp_file:
...
class CustomerPage(MenuPage):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Button(self,image=self.menu, text="Menu",compound="top",bg="blue",borderwidth=0,
            command=lambda:controller.show_frame("MenuPage")).place(x=0,y=525)
        tk.Button(self,image=self.submit_order,text="Submit Order",compound="top",bg='blue',
            borderwidth=0,command=self.submit_order_fn).place(x=400,y=525)
        tk.Button(self, image=self.exit,compound="top",bg="blue",borderwidth=0,
            text="Exit", command=self.exit_fn).place(x=800,y=525)
...
class ClockPage(CustomerPage):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Button(self, image=self.exit,compound="top",bg='blue',borderwidth=0,
            text="Exit", command=lambda: controller.show_frame("StartPage")).place(x=800,y=525)
...
class ManagerPage(ClockPage):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Button(self,text="Show Inventory",image=self.note,compound='top',bg='blue',borderwidth=0,
            command=self.show_inventory).place(x=150,y=250)
        tk.Button(self, text="Update Inventory",image=self.update_inv,compound='top',bg='blue',
            borderwidth=0,command=self.update_inv_fn).place(x=400,y=250)
        tk.Button(self, text="Show Orders",image=self.note,compound='top',bg='blue',
            borderwidth=0,command=self.show_order_fn).place(x=250,y=0)
        tk.Button(self, text="Show Customer",image=self.emp_img,compound='top',bg='blue',
            borderwidth=0,command=self.show_customer_fn).place(x=550,y=0)
        tk.Button(self, text="Employee Logs",image=self.emp_img,compound='top',bg='blue',
            borderwidth=0,command=self.employee_log_fn).place(x=650,y=250)
        tk.Button(self, text="Exit",image=self.exit,compound='top',bg='blue',borderwidth=0,
            command=lambda:controller.show_frame("StartPage")).place(x=800,y=525)
    def employee_log_fn(self):
        ...
        #Quit button
        tk.Button(emp_win,bg="red",text="Quit",command=emp_win.destroy).place(x=967,y=725)
    ...
    def show_customer_fn(self):
        ...
        #Create new window to show order
        cus_win = tk.Toplevel()
        cus_win.title("Show Order")
        cus_win.geometry("1000x750")
        cus_win.configure(background = "blue")

        #Creating scrollbar and listbox to add order to
        scrollbar=tk.Scrollbar(cus_win,orient=VERTICAL)
        scrollbar.pack(side=RIGHT, fill=Y,pady=25)
        list_box=tk.Listbox(cus_win,background="blue",yscrollcommand=scrollbar.set,
            borderwidth=0,highlightthickness=0)
        for line in lines:
            list_box.insert(END,line)
        file.close()
        ...
        #Quit button
        tk.Button(cus_win,bg="red",text="Quit",command=cus_win.destroy).place(x=967,y=725)
    ...
    def show_order_fn(self):
        ...
        #Create new window to show order
        ord_win = tk.Toplevel()
        ord_win.title("Show Order")
        ord_win.geometry("1000x750")
        ord_win.configure(background = "blue")

        #Creating scrollbar and listbox to add order to
        scrollbar=tk.Scrollbar(ord_win,orient=VERTICAL)
        scrollbar.pack(side=RIGHT, fill=Y,pady=25)
        list_box=tk.Listbox(ord_win,background="blue",yscrollcommand=scrollbar.set,
            borderwidth=0,highlightthickness=0)
        for line in lines:
            list_box.insert(END,line)
        file.close()
        ...
        #Quit button
        tk.Button(ord_win,bg="red",text="Quit",command=ord_win.destroy).place(x=967,y=725)
